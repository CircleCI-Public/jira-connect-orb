...
@test "Basic expansion works" {
  ...
  assert_jq_contains '.jobs["build"].steps[3].run.command' 'cannonical="/rest/builds/0.1/bulk"'
...
@test "Basic expansion for deployments" {
  ...
  assert_jq_match '.jobs["build"].steps[0].run.command' 'echo "hello"'
  assert_jq_match '.jobs["build"].steps[3].run.name' 'Update status in Atlassian Jira'
  assert_jq_contains '.jobs["build"].steps[3].run.command' 'cannonical="/rest/deployments/0.1/bulk"'
}
